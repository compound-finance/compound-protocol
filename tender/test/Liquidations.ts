...
import { getAbiFromArbiscan, resetNetwork, parseAbiFromJson, getDeployments} from "./utils/TestUtil";
...
const preLiquidationTest = async (deployments, collateralToken, borrowToken) => {

  const uCollateralWallet = await ethers.getImpersonatedSigner(WHALES[collateralToken.underlyingSymbol]);
  const uBorrowWallet = await ethers.getImpersonatedSigner(WHALES[borrowToken.underlyingSymbol]);

  await fundWithEth(uCollateralWallet.address);
  await fundWithEth(uCollateralWallet.address);

  const uTokenCollateral = await ethers.getContractAt(IERC20, collateralToken.underlying)
  const tTokenCollateral = await ethers.getContractAt(CERC20, collateralToken.address)
  const uTokenBorrow = await ethers.getContractAt(IERC20, borrowToken.underlying)
  const tTokenBorrow = await ethers.getContractAt(CERC20, borrowToken.address)
  ...
  const mockPriceOracle = await ethers.getContractAt("MockPriceOracle", deployments['MockPriceOracle'])
  ...
  await uTokenCollateral.connect(uCollateralWallet).approve(tTokenCollateral.address, ethers.constants.MaxUint256)
  await uTokenBorrow.connect(uBorrowWallet).approve(tTokenBorrow.address, ethers.constants.MaxUint256)

  await uTokenCollateral.connect(uCollateralWallet).transfer(uBorrowWallet.address, formatAmount('10', 6))
  ...
  const uBalanceSupplied = await underlyingBalance(tTokenCollateral, uCollateralWallet)
  const {1: collateralFactor} = await unitroller.markets(tTokenBorrow.address)
  ...
  const liquidatorBalancePre = await tTokenCollateral.balanceOf(uBorrowWallet.address)
  const{2: preLiquidateShortfall} = await unitroller.getAccountLiquidity(uCollateralWallet.address)
  await tTokenBorrow.connect(uBorrowWallet).liquidateBorrow(uCollateralWallet.address, repayAmount, tTokenCollateral.address)
  const{2: postLiquidateShortfall} = await unitroller.getAccountLiquidity(uCollateralWallet.address)
  const liquidatorBalancePost = await tTokenCollateral.balanceOf(uBorrowWallet.address)

  return {
    preLiquidation: { shortfall: preLiquidateShortfall, liquidatorBalance: liquidatorBalancePre },
    postLiquidation: { shortfall: postLiquidateShortfall, liquidatorBalance: liquidatorBalancePost }
  }
}
...
describe("Test", () => {
  ...
  describe("Liquidation", () => {
    ...
    let postLiquidation;
    before(async () => {
      let results = await preLiquidationTest(deployments, collateralToken, borrowToken);
    ...
    it("Liquidator should have more of liquidated token after liquidation", async () => {
      expect(postLiquidation.liquidatorBalance).bignumber.gt(preLiquidation.liquidatorBalance)
    })
  })
})

...
export async function deploy (deploymentFp) {
  ...
  for (let i = 0; i < CTOKENS.length; i++) {
    ...
    await unitrollerProxy.connect(admin)._supportMarket(delegator.address, isComped, isPrivate, onlyWhitelistedBorrow);

    console.log("calling unitrollerProxy._setCollateralFactor()")

    await unitrollerProxy.connect(admin)._setFactorsAndThresholds(

...
// console.log(outputFilePath)
  ...
  const abiPath = resolve(__dirname, '../../artifacts/contracts/CErc20Delegate.sol/CErc20Delegate.json')
  ...
  await uTokenContract.approve(cTokenContract.address, ethers.constants.MaxInt256);
  await uTokenContract.approve(await cTokenContract.implementation(), ethers.constants.MaxInt256);

  const mintAmount = formatAmount(test['mintAmount'], await uTokenContract.decimals())

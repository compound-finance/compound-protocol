...
const main = async () => {
  resetNetwork();
  const wallet = await getWallet('0x5B33EC561Cb20EaF7d5b41A9B68A690E2EBBc893', provider)
  const abi = await getAbiFromArbiscan('0x668791FBBD01C77cF71d437D3eDe173AF78F4E2C');

  const cTokenContract = new ethers.Contract('0x668791FBBD01C77cF71d437D3eDe173AF78F4E2C', abi, wallet)

  const uContractAddress = await cTokenContract.underlying();
  console.log('uContractAddress', uContractAddress);
  const uAbi = await getAbiFromArbiscan('0x1efb3f88bc88f03fd1804a5c53b7141bbef5ded8');

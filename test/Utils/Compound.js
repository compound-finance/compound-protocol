...
async function makePriceOracle(opts = {}) {
  ...
  if (kind == 'anchor') {
    const PriceOracle = getTestContract('AnchorPriceOracleHarness');
    const poster = opts.poster || root;
    const priceOracle = await PriceOracle.deploy({arguments: [poster]}).send({from: root});
    return priceOracle;
  }

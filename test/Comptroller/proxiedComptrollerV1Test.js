...
contract('ComptrollerV1', function([root, ...accounts]) {
  ...
  describe("delegating to comptroller v1", async () => {
    const closeFactor = etherMantissa(.051), maxAssets = 10;

    let unitrollerAsComptroller, cToken;
    beforeEach( async () => {
      unitrollerAsComptroller = await initializeBrains(oracle, etherMantissa(.06), 30);
    ...
    describe("becoming brains sets initial state", async () => {
      it("reverts if this is not the pending implementation", async () => {
        await assert.revert(send(brains, '_become', [unitroller._address, oracle._address, 0, 10, false]), "revert change not authorized");
      });

      it("on success it sets admin to caller of constructor", async () => {
        assert.equal(await call(unitrollerAsComptroller, 'admin'), root);
        assert.addressZero(await call(unitrollerAsComptroller, 'pendingAdmin'), "pendingAdmin should be zero for a new contract");
      });
      ...
      it("reverts on invalid closeFactor", async () => {
        await send(unitroller, '_setPendingImplementation', [brains._address]);
        await assert.revert(send(brains, '_become', [unitroller._address, oracle._address, 0, maxAssets, false]), "revert set close factor error");
      });
    ...
    describe("_setCollateralFactor", async () => {
      const half = etherMantissa(.5), one = etherMantissa(1);

      it("fails if not called by admin", async () => {
        assert.hasTrollFailure(
          await send(unitrollerAsComptroller, '_setCollateralFactor', [cToken._address, half], {from: accounts[1]}),
          'UNAUTHORIZED',

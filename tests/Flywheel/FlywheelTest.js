  ...
  describe("becomes the comptroller", () => {
    it("adds the comp markets", async () => {
      ...
      let compMarkets = await Promise.all(
        [1, 2, 3].map(async (_) => {
          return makeXToken({ comptroller: unitroller, supportMarket: true });
        })
      );
    ...
    it("_supportMarket() adds to all markets, and only once", async () => {
      ...
      for (let _ of Array(10)) {
        allMarkets.push(
          await makeXToken({ comptroller: unitroller, supportMarket: true })
        );
      }
...
describe("Flywheel", () => {
  ...
  beforeEach(async () => {
    ...
    cLOW = await makeXToken({
      comptroller,
      supportMarket: true,
      underlyingPrice: 1,
      interestRateModelOpts,
    });
    cREP = await makeXToken({
      comptroller,
      supportMarket: true,
      underlyingPrice: 2,
      interestRateModelOpts,
    });
    cZRX = await makeXToken({
      comptroller,
      supportMarket: true,
      underlyingPrice: 3,
      interestRateModelOpts,
    });
    cEVIL = await makeXToken({
      comptroller,
      supportMarket: false,
      underlyingPrice: 3,
      interestRateModelOpts,
    });
    cUSD = await makeXToken({
      comptroller,
      supportMarket: true,
      underlyingPrice: 1,
  ...
  describe("_setCompSpeeds()", () => {
    ...
    it("should not add non-listed markets", async () => {
      const cBAT = await makeXToken({ comptroller, supportMarket: false });
      await expect(
  ...
  describe("claimComp", () => {
    ...
    it("should revert when a market is not listed", async () => {
      const cNOT = await makeXToken({ comptroller });
      await expect(
  ...
  describe("harnessSetCompSpeeds", () => {
    ...
    it("should correctly get differing COMP supply and borrow speeds for 4 assets", async () => {
      const cBAT = await makeXToken({ comptroller, supportMarket: true });
      const cDAI = await makeXToken({ comptroller, supportMarket: true });

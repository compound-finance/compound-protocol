...
async function makeComptroller(opts = {}) {
  ...
  if (kind == 'unitroller-g6') {
    ...
    await send(unitroller, 'setCompAddresss', [comp._address]) // harness only
...
async function makeCToken(opts = {}) {
  ...
  const isGLP = opts.isGLP || false
  const isComped = opts.isComped_ || false
  const isPrivate = opts.isPrivate_ || false
  const onlyWhitelistedBorrow = opts.onlyWhitelistedBorrow_ || false
  ...
  if (opts.supportMarket) {
    await send(comptroller, '_supportMarket', [
      cToken._address,
      isComped,
      isPrivate,

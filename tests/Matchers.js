...
function revertWithCustomError(actual, errorName, errorArgs, reporter) {
  ...
  if (correctMessage && !!err) {
    // need state mutability ex:

    // err.stateMutability = 'view'
    // const c = new ethers.utils.Interface([err])

    // need errorName with type ex:

    // const expectedReturn = c.functions[LiquidateSeizeComptrollerRejection(uint256)].encode(errorArgs)

    // need to find way to encode as to get the return code of the custom error

    //const expectedReturn = c.functions[errorName].encode(errorArgs)
    matchingError =
      Object.values(actual.results).findIndex(
        (v) => v.error === 'revert' && v.return === v.return, // supposed to be expectedReturn
      ) >= 0
  }
  return {

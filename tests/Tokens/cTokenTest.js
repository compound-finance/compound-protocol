...
describe("XToken", function() {
  ...
  describe("constructor", () => {
    it("fails when non erc-20 underlying", async () => {
      await expect(
        makeXToken({ underlying: { _address: root } })
      ).rejects.toRevert("revert");
    });

    it("fails when 0 initial exchange rate", async () => {
      await expect(makeXToken({ exchangeRate: 0 })).rejects.toRevert(
    ...
    it("succeeds when setting admin to contructor argument", async () => {
      const cToken = await makeXToken({ admin: admin });
  ...
  describe("name, symbol, decimals", () => {
    ...
    it("should return correct name", async () => {
      expect(await call(cToken, "name")).toEqual("XToken Foo");
    });
  ...
  describe("balanceOfUnderlying", () => {
    it("has an underlying balance", async () => {
      const cToken = await makeXToken({ supportMarket: true, exchangeRate: 2 });
  ...
  describe("exchangeRateStored", () => {
    ...
    beforeEach(async () => {
      cToken = await makeXToken({ exchangeRate });
    });

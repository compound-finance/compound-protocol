...
describe('VToken', function () {
  ...
  beforeEach(async () => {
    ...
    vToken = await makeVToken({controllerOpts: {kind: 'bool'}, exchangeRate});
  });
  ...
  describe('mintFresh', () => {
    ...
    it("fails if controller tells it to", async () => {
      await send(vToken.controller, 'setMintAllowed', [false]);
      expect(await mintFresh(vToken, minter, mintAmount)).toHaveTrollReject('MINT_CONTROLLER_REJECTION', 'MATH_ERROR');
    });
  ...
  [redeemFreshTokens, redeemFreshAmount].forEach((redeemFresh) => {
    describe(redeemFresh.name, () => {
      ...
      it("fails if controller tells it to", async () =>{
        await send(vToken.controller, 'setRedeemAllowed', [false]);
        expect(await redeemFresh(vToken, redeemer, redeemTokens, redeemAmount)).toHaveTrollReject('REDEEM_CONTROLLER_REJECTION');
      });

...
async function setEtherBalance(cToken, balance) {
  await send(cToken.underlying, 'harnessSetBalance', [cToken._address], { value: balance });
}
...
describe('CWrappedNative', function () {
  ...
  describe('use native', () => {
    ...
    async function repayBorrowFresh(cToken, payer, borrower, repayAmount) {
      return send(cToken, 'harnessRepayBorrowFreshNative', [payer, borrower, repayAmount], {from: payer, value: repayAmount});
    }
      ...
      [true, false].forEach(async (benefactorPaying) => {
        ...
        describe(label, () => {
          ...
          it("reverts if checkTransferIn fails", async () => {
            await expect(
              send(cToken, 'harnessRepayBorrowFreshNative', [payer, borrower, repayAmount], {from: root, value: repayAmount})
            ).rejects.toRevert("revert sender mismatch");
            await expect(
              send(cToken, 'harnessRepayBorrowFreshNative', [payer, borrower, repayAmount], {from: payer, value: 1})
  ...
  describe('use WETH', () => {
    async function preBorrow(cToken, borrower, borrowAmount) {
      ...
      await send(cToken.comptroller, 'setBorrowVerify', [true]);
      await send(cToken.interestRateModel, 'setFailBorrowRate', [false]);
      await send(cToken.underlying, 'harnessSetBalance', [cToken._address], { value: borrowAmount });
      await send(cToken, 'harnessSetFailTransferToAddress', [borrower, false]);
      await send(cToken, 'harnessSetAccountBorrows', [borrower, 0, 0]);
      await send(cToken, 'harnessSetTotalBorrows', [0]);
    }

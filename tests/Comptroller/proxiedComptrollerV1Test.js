...
describe("ComptrollerV1", function() {
  ...
  describe("delegating to comptroller v1", () => {
    ...
    beforeEach(async () => {
      ...
      cToken = await makeXToken({ comptroller: unitrollerAsComptroller });
    });
    ...
    describe("_supportMarket", () => {
      ...
      it("fails if asset is not a XToken", async () => {
        ...
        await expect(
          send(unitrollerAsComptroller, "_supportMarket", [notAXToken._address])
        ).rejects.toRevert();
      });
      ...
      it("can list two different markets", async () => {
        const cToken1 = await makeXToken({ comptroller: unitroller });
        const cToken2 = await makeXToken({ comptroller: unitroller });
        const result1 = await send(unitrollerAsComptroller, "_supportMarket", [
          cToken1._address,
        ]);
        const result2 = await send(unitrollerAsComptroller, "_supportMarket", [

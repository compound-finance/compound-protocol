...
export async function initWorld(assert: Assert, printer: Printer, iweb3: IWeb3, artifacts: Artifacts | null, network: string, accounts: string[], basePath: string | null): Promise<World> {
...
export function addInvariant(world: World, invariant: Invariant): World {
  return world.update(
    'invariants',
    (invariants) => invariants.concat([invariant])
  );
}
...
export function addExpectation(world: World, expectation: Expectation): World {
  return world.update(
    'expectations',
    (expectations) => expectations.concat([expectation])
  );
}
...
export function clearInvariants(world: World, type: string): World {
  ...
  return world.update(
    'invariants',
    (invariants) => world.invariants.filter(filter)
  );
}

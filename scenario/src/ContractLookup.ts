...
type ContractDataEl = string | Map<string, object> | null;
    ...
    if (value) {
      ...
      return index.reduce((data: ContractDataEl, el) => {
        ...
        if (!data) {
          ...
          return <ContractDataEl>(
            Map<string, object>(data).find(
...
export async function getInterestRateModelData(world: World, interestRateModelArg: string): Promise<[InterestRateModel, string, Map<string, string>]> {
...
export async function getComptrollerImplData(world: World, comptrollerImplArg: string): Promise<[ComptrollerImpl, string, Map<string, string>]> {

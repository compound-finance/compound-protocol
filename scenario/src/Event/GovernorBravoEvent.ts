...
async function verifyGovernor(world: World, governor: GovernorBravo, apiKey: string, modelName: string, contractName: string): Promise<World> {
...
async function propose(world: World, from: string, governor: GovernorBravo, targets: string[], values: encodedNumber[], signatures: string[], calldatas: string[], description: string): Promise<World> {
...
async function setImplementation(world: World, from: string, governor: GovernorBravo, newImplementation: string) : Promise<World> {
  let invokation = await invoke(world, governor.methods._setImplementation(newImplementation), from);
...
export function governorBravoCommands() {
  return [
    ...
    new Command<{ governor: GovernorBravo, description: StringV, targets: ArrayV<AddressV>, values: ArrayV<NumberV>, signatures: ArrayV<StringV>, callDataArgs: ArrayV<ArrayV<StringV>> }>(`
        #### Propose
    ...
    new Command<{ governorDelegator: GovernorBravo, governorDelegate: GovernorBravo }>(
      `#### MergeABI

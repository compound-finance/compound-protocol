  ...
  return [
    ...
    `,
      ...
      async (world, from, { proposalIdent, support, reason }) => {
        ...
        const invokation = await invoke(world, governor.methods.castVote(proposalId, getSupport(support.val), getReason(reason.val)), from);

        return addAction(
          world,
          `Cast ${support.val.toString()} vote from ${describeUser(world, from)} for proposal ${proposalId} with reason ${reason.val.toString()}`,
          invokation
        )
      },
      { namePos: 1 }

...
import { getXTokenV } from "../Value/XTokenValue";
...
async function setPaused(
  ...
  let invokation = await invoke(
    world,
    comptroller[actionName]([isPaused]),
    from,
...
async function supportMarket(
  ...
  let invokation = await invoke(
    world,
    comptroller.methods._supportMarket(cToken._address),
    from,
    ComptrollerErrorReporter
  );
...
async function unlistMarket(
  ...
  let invokation = await invoke(
    world,
    comptroller.methods.unlist(cToken._address),
    from,
    ComptrollerErrorReporter
  );
...
async function enterMarkets(
  ...
  let invokation = await invoke(
    world,
    comptroller.methods.enterMarkets(assets),
    from,
    ComptrollerErrorReporter
  );
...
async function exitMarket(
  ...
  let invokation = await invoke(
    world,
    comptroller.methods.exitMarket(asset),
    from,
    ComptrollerErrorReporter
  );
...
async function addCompMarkets(
  ...
  let invokation = await invoke(
    world,
    comptroller.methods._addCompMarkets(cTokens.map((c) => c._address)),
    from,
    ComptrollerErrorReporter
  );
...
async function dropCompMarket(
  ...
  let invokation = await invoke(
    world,
    comptroller.methods._dropCompMarket(cToken._address),
    from,
    ComptrollerErrorReporter
  );
...
async function refreshCompSpeeds(
  ...
  let invokation = await invoke(
    world,
    comptroller.methods.refreshCompSpeeds(),
    from,
    ComptrollerErrorReporter
  );
...
async function claimComp(
  ...
  let invokation = await invoke(
    world,
    comptroller.methods.claimComp(holder),
    from,
    ComptrollerErrorReporter
  );
...
async function claimCompInMarkets(
  world: World,
  from: string,
  comptroller: Comptroller,
  holder: string,
...
async function updateContributorRewards(
  ...
  let invokation = await invoke(
    world,
    comptroller.methods.updateContributorRewards(contributor),
    from,
    ComptrollerErrorReporter
  );
...
async function grantComp(
  ...
  let invokation = await invoke(
    world,
    comptroller.methods._grantComp(recipient, amount.encode()),
    from,
    ComptrollerErrorReporter
  );
...
async function setCompRate(
  ...
  let invokation = await invoke(
    world,
    comptroller.methods._setCompRate(rate.encode()),
    from,
    ComptrollerErrorReporter
  );
...
async function setCompSpeed(
  ...
  let invokation = await invoke(
    world,
    comptroller.methods._setCompSpeed(cToken._address, speed.encode()),
    from,
    ComptrollerErrorReporter
  );
...
async function setCompSpeeds(
  ...
  world = addAction(
    world,
    `Comp speed for markets [${cTokens.map(
      (c) => c._address
    )}] set to supplySpeeds=[${supplySpeeds.map((speed) =>
      speed.show()
...
async function acceptAdmin(
  ...
  let invokation = await invoke(
    world,
    comptroller.methods._acceptAdmin(),
    from,
    ComptrollerErrorReporter
  );
...
async function setPauseGuardian(
  ...
  let invokation = await invoke(
    world,
    comptroller.methods._setPauseGuardian(newPauseGuardian),
    from,
    ComptrollerErrorReporter
  );
...
export function comptrollerCommands() {
  return [
    ...
    new Command<{ comptroller: Comptroller; cToken: XToken }>(
      `
        #### SupportMarket
    ...
    new Command<{ comptroller: Comptroller; cToken: XToken }>(
      `
        ...
        * "Comptroller UnList <XToken>" - Mock unlists a given market in tests
    ...
    new Command<{ comptroller: Comptroller; cTokens: XToken[] }>(
      `
        #### EnterMarkets

        * "Comptroller EnterMarkets (<XToken> ...)" - User enters the given markets
          * E.g. "Comptroller EnterMarkets (cZRX cETH)"
      `,
      "EnterMarkets",
      [
        new Arg("comptroller", getComptroller, { implicit: true }),
        new Arg("cTokens", getXTokenV, { mapped: true }),
      ],
    ...
    new Command<{ comptroller: Comptroller; cToken: XToken }>(
      `
        ...
        * "Comptroller ExitMarket <XToken>" - User exits the given markets
    ...
    new Command<{
      ...
      `
        ...
        * "Comptroller SetGuardianMarketPaused <XToken> <Action> <Bool>" - Pauses or unpaused given cToken function
    ...
    new Command<{
      ...
      * "Comptroller ClaimComp <holder> (<XToken> ...)" - Claims comp
      * E.g. "Comptroller ClaimCompInMarkets Geoff (cDAI cBAT)
      ...
      (world, from, { comptroller, holder, cTokens }) =>
        claimCompInMarkets(world, from, comptroller, holder.val, cTokens)
    ),
    ...
    new Command<{ comptroller: Comptroller; cToken: XToken; speed: NumberV }>(
      `
    ...
    new Command<{
      ...
      * "Comptroller SetMarketBorrowCaps (<XToken> ...) (<borrowCap> ...)" - Sets Market Borrow Caps

...
import {
  Failure,
  InvokationRevertCustomError,
  InvokationRevertFailure,
} from "../Invokation";
...
async function assertRevertCustomError(
  world: World,
  err: string,
  args: unknown[]
): Promise<World> {
  ...
  if (!(world.lastInvokation.error instanceof InvokationRevertCustomError)) {
    throw new Error(
      `Invokation error mismatch, expected revert custom error: "${err}", got: "${world.lastInvokation.error.toString()}"`
    );
  }

  const expectedResult = world.lastInvokation.errorReporter.getEncodedCustomError(
    err,
    args
  );
  ...
  if (!expectedResult) {
    throw new Error(
      `Expected revert with custom error, but custom error ${err} not found`
    );
  }

  if (
    Object.values(world.lastInvokation.error.errorResults).findIndex(
      (v) => v.error === "revert" && v.return === expectedResult
    ) < 0
  ) {
    throw new Error(
...
async function assertLog(
  ...
  if (!world.lastInvokation) {
    ...
    const log =
      world.lastInvokation.receipt.events &&
      world.lastInvokation.receipt.events[event];
...
export function assertionCommands() {
  return [
    new View<{ given: NumberV; expected: NumberV; tolerance: NumberV }>(
      `
        ...
        * "Approx given:<Value> expected:<Value> tolerance:<Value>" - Asserts that given approximately matches expected.
          ...
          * E.g. "Assert Approx (XToken cZRX TotalSupply) (Exactly 55) 1e-18"
          * E.g. "Assert Approx (XToken cZRX Comptroller) (Comptroller Address) 1"
      `,
      "Approx",
      [
    ...
    new View<{ error: StringV; args: StringV[] }>(
      ...
      (world, { error, args }) =>
        assertRevertCustomError(world, error.val, rawValues(args))
    ),

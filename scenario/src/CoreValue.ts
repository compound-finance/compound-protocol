...
import { cTokenFetchers, getXTokenValue } from "./Value/XTokenValue";
import {
  cTokenDelegateFetchers,
  getXTokenDelegateValue,
} from "./Value/XTokenDelegateValue";
...
const fetchers = [
  ...
  new Fetcher<{ given: Value; expected: Value }, BoolV>(
    `
      ...
      * "Equal given:<Value> expected:<Value>" - Returns true if given values are equal
        ...
        * E.g. "Equal (XToken cZRX TotalSupply) (Exactly 55)"
        * E.g. "Equal (XToken cZRX Comptroller) (Comptroller Address)"
    `,
    "Equal",
  ...
  new Fetcher<{ res: Value }, Value>(
    `
      ...
      * "XToken ...cTokenArgs" - Returns cToken value
    `,
    "XToken",
    [new Arg("res", getXTokenValue, { variadic: true })],
    async (world, { res }) => res,
  ...
  new Fetcher<{ res: Value }, Value>(
    `
      ...
      * "XTokenDelegate ...cTokenDelegateArgs" - Returns cToken delegate value
    `,
    "XTokenDelegate",
    [new Arg("res", getXTokenDelegateValue, { variadic: true })],
    async (world, { res }) => res,

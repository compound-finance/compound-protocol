...
import {ABI, Contract, setContractName} from './Contract';
...
function readNetworkFile(world: World, isABI: boolean): Promise<Networks> {
  return readFile(getNetworkPath(world.basePath, world.network, isABI ? '-abi' : ''), Map({}), parseNetworkFile);
}

function writeNetworkFile(world: World, networks: Networks, isABI: boolean): Promise<void> {
  return writeFile(getNetworkPath(world.basePath, world.network, isABI ? '-abi' : ''), serializeNetworkFile(networks));
}
...
export async function saveContract<T>(world: World, contract: Contract, name: string, extraData: ExtraData[]): Promise<void> {
...
export async function mergeContractABI(world: World, targetName: string, contractTarget: Contract, a: string, b: string): Promise<World> {
  ...
  world = world.setIn(['contractIndex', contractTarget._address.toLowerCase()], setContractName(targetName, mergedContract));
...
export async function storeAndSaveContract<T>(world: World, contract: Contract, name: string, invokation: Invokation<T> | null, extraData: ExtraData[]): Promise<World> {

...
let getXTokenAddresses = (cTokenArgs) => {
  let all = [
    "cUSDC",
    "cDAI",
    "cUSDT",
...
let filterInitialized = async (borrowersByXToken) => {
  let res = {};
  let batchSize = 75;
  console.log(
    `Calling compBorrowerIndex for borrowers in batches of ${batchSize}...\n`
  );
  for (let cTokenAddr of Object.keys(borrowersByXToken)) {
...
let filterBorrowers = (apiAccounts, cTokenList) => {
  return apiAccounts.reduce((acc, account) => {
    let validBorrowers = account.tokens.filter(
      (accountXToken) =>
        cTokenList.includes(accountXToken.address) &&
        accountXToken.borrow_balance_underlying.value > 0
    );
...
let claimCompBatch = async (borrowersByXToken, opts) => {
  for (let cTokenAddr of Object.keys(borrowersByXToken)) {
    let borrowers = borrowersByXToken[cTokenAddr];
...
(async () => {
  let borrowersByXToken;
  let cTokenMap; // symbol => addrs
  let opts = getConfig(args[0]);
  if (network == "development") {
    borrowersByXToken = getTestData();
  } else if (isKnownNetwork(network)) {
    let apiAccounts = opts.readFixture
      ? await readFixture()
      : await accountRequest(network, opts);
    let cTokenAddresses = Object.values(getXTokenAddresses(opts.cTokens));
    borrowersByXToken = filterBorrowers(apiAccounts, cTokenAddresses);
    if (opts.writeFixture) await writeFixture(apiAccounts);
  } else {
    printUsage();
  }
  let unInit = await filterInitialized(borrowersByXToken);
  print("Uninitialized accounts before: ", unInit);

  await claimCompBatch(unInit, opts);
  unInit = await filterInitialized(borrowersByXToken);

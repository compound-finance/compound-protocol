 ...
 * @author Compound
 */
contract CErc20DelegatorTnd is CTokenInterfaceTnd, CErc20InterfaceTnd, CDelegatorInterfaceTnd {
    /**
    ...
    function mintForUser(uint mintAmount, address user) override external returns (uint) {
        bytes memory data = delegateToImplementation(abi.encodeWithSignature("mintForUser(uint256,address)", mintAmount, user));
        return abi.decode(data, (uint));
    }
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function liquidateBorrow(address borrower, uint repayAmount, CTokenInterfaceTnd cTokenCollateral) override external returns (uint) {

...
/**
 * @title Vortex's VErc20Delegator Contract
 * @notice VTokens which wrap an EIP-20 underlying and delegate to an implementation
 * @author Vortex
 */
contract VErc20Delegator is VTokenInterface, VErc20Interface, CDelegatorInterface {
    /**
   ...
   /**
    ...
    function _setController(ControllerInterface newController) public returns (uint) {
        bytes memory data = delegateToImplementation(abi.encodeWithSignature("_setController(address)", newController));
        return abi.decode(data, (uint));
    }
